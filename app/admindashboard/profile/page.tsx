...
import AdminProfileModal from '@/components/AdminDashboardModal/AdminProfileModal';
...
interface AdminProfileData {
    ...
    lastname: string;
    email: string;
    password: string; // Consider whether you want to display this
    address: string;
    phoneNumber: string;
}

const AdminDashboardProfile: React.FC = () => {
    const [profileData, setProfileData] = useState<AdminProfileData | null>(null);
    const [isModalOpen, setModalOpen] = useState(false);

    const handleSave = (data: AdminProfileData) => {
        setProfileData(data);
    };
        ...
        <Layout>
            <div className="shadow-md rounded-md p-6 bg-appBanner h-[78vh] overflow-y-scroll">
                <div className="flex items-center justify-between w-full border-b-2 border-b-appTitleBgColor pb-1">
                    <h2 className="text-2xl font-semibold text-white font-serif"> Admin Profile Page </h2>
                    <button
                        onClick={toggleModal}
                        className="bg-appTitleBgColor transition-colors text-white py-2 px-4 rounded-md"
                    >
                        Edit Profile
                    </button>
                </div>
