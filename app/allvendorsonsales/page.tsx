...
import { Reviews, VendorDescHolder, VendorOnSalesDescHolder, VendorsBanner, VenorsTags } from "@/data/data";
import Image from "next/image";
import Link from "next/link";
import React from "react";
const AllVendorsOnSales = () => {
    return (
        <div className="w-full h-[100vh - 6.5rem] flex flex-col justify-center items-center gap-2 py-1 sm:gap-4">
            ...
            { VendorOnSalesDescHolder && (
                VendorOnSalesDescHolder.map((vendor)=> (
                    <div className="relative rounded-xl w-[150px] shadow-md sm:w-[200px]">
                        <Image src={vendor.image} alt="" width={155} height={150} className="object-contain sm:w-[200px]"/>
                        <Link href={vendor.link}>
                        <div className="flex justify-center items-center">
                        <button className="absolute top-[170px] w-[100px] h-[35px] bg-[#4d7b7b] items-center rounded-xl text-white font-inter font-light sm:top-[235px]">Shop Now</button>
                        </div>
                        </Link>
                    </div>
...
export default AllVendorsOnSales